      ...
      <div className={css.baner__changed}>
        ...
        {(searchRes===0 || searchRes) && (
          <h1 className={css.baner__text}>
            Знайдено за результатами пошуку: <br/> <span className={css.activeRes} >{searchRes} документів</span>
          </h1>
        )}
       
      </div>
      {docName && <div className={css.docWrapper}><h1 className={css.docName}>{docName}</h1></div>}
    </div>
  );
}
